// Тебя я тем более потом доделаю и переведу. Время 1 час ночи, вы что ебобо Андрей?!
...
/* // Позволяет пить с рук, но сильно может сажать ФПС :С
...
// Буксировка (вертолетами) стандартная...
...
// Увеличим дистанцию только для AIR или SHIP
...
if (!isNull _cursorTarget && !_inVehicle && !_isPZombie && (player distance _cursorTarget < _allowedDistance) && _canDo) then
{
	...
	//_isVehicletype = _typeOfCursorTarget in ["ATV_US_EP1","ATV_CZ_EP1"]; // Используется для player_flipvehicle
	...
	if ((_cursorTarget call isInflamed) or (inflamed _cursorTarget)) then
	{
		...
		//_hastinitem = {_x in boil_tin_cans} count _magazinesPlayer > 0;	// Чтобы переплавлять банки?
	...
	if (_isAlive) then
	{
		...
		// Игроки могут убирать объекты без разрешения
		...
		// Игроки могут убирать объекты только с разрешения
	...
	// Убираем прочие ownerID объекты
...
*/
	...
	// Позволяем Овнерам открыть/закрывать технику  
	...
	// Позвляем Овнерам открывать Сейф
	...
	// Позвляем Овнерам паковать Сейф
	...
	// Для улучшения построек (Модульных)
	...
	// Для ДаунГрейда построек (Модульных)
	...
	if ((damage _cursorTarget >= DZE_DamageBeforeMaint) && {_cursorTarget isKindOf "ModularItems" || _cursorTarget isKindOf "DZE_Housebase" || _typeOfCursorTarget == "LightPole_DZ"}) then
	{
		...
		if (s_player_maint_build < 0) then
		{
			...
			if ((_hasAccess select 0) or (_hasAccess select 2) or (_hasAccess select 3)) then
			{
				_text2 = _text + " (" + str(round ((damage _cursorTarget) * 100)) + "% повреждений)";
	...
	*/
	...
	if (_isMan && {!(isPlayer _cursorTarget)} && {_typeOfCursorTarget in serverTraders} && {!_isPZombie}) then
	{
		if (s_player_parts_crtl < 0) then
		{
			...
			{
				// Статичное меню (не GUI)
				{
					_buy = player addAction [format["Обменять %1 %2 на %3 %4",(_x select 3),(_x select 5),(_x select 2),(_x select 6)], "\z\addons\dayz_code\actions\trade_items_wo_db.sqf",[(_x select 0),(_x select 1),(_x select 2),(_x select 3),(_x select 4),(_x select 5),(_x select 6)], (_x select 7), true, true];
	...
	if (dayz_tameDogs) then
	{
		...
		if (_isDog && {_ownerID == _id} && {_isAlive}) then
		{
			...
			{
				if (_dogHandle getFSMVariable "_actionLieDown") then
				{
					_text = "str_actions_liedog";
				}
				else
				{
					_text = "str_actions_sitdog";
				};
			...
			if (s_player_warndog < 0) then
			{
				...
				if (_warn) then
				{
					_text = localize "str_epoch_player_247";
					_warn = false;
				}
				else
				{
					_text = localize "str_epoch_player_248";
					_warn = true;
				};
...
if (_dogHandle > 0) then
{
	...
	if (_canDo && !_inVehicle && alive _dog && !(_ownerID in [dayz_characterID,_uid])) then
	{
		...
		if (s_player_speeddog < 0) then
		{
			...
			if (_dog getVariable ["currentSpeed",1] == 0) then
			{
				_speed = 1;
				_text = localize "str_epoch_player_250";
			};
