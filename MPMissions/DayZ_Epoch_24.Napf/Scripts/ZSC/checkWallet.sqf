...
{
	isPlayer _x
} count ((getPosATL _body) nearEntities ["CAManBase", 10]) > 1;
...
if (isNil "_hisMoney") then
{
	_hisMoney = 0;
};

if (!(typeName _hisMoney == "SCALAR") || {_hisMoney < 1}) exitWith
{
	dayz_actionInProgress = false;
	format [localize "STR_ZSC_NO_COINS",_name,CurrencyName] call dayz_rollingMessages
};
