...
BankDialogUpdateAmounts =
{
	...
	if (isClass(configFile >> "CfgVehicles" >> _vehicleType)) then
	{
		...
		if (_vehicleMagazines == 0) then
		{
			_vehicleMagazines = ZSC_defaultStorageMultiplier
		};
...
BankDialogWithdrawAmount =
{
	...
	if (!isNull ZSC_CurrentStorage) then
	{
		...
		{
			format [localize "STR_ZSC_WITHDRAW_FAIL",CurrencyName,_displayName] call dayz_rollingMessages;
		};
...
BankDialogDepositAmount =
{
	...
	{
		format [localize "STR_ZSC_DEPOSIT_FAIL",CurrencyName] call dayz_rollingMessages;
	};
...
GivePlayerAmount =
{
	...
	if ((_amount < 1) or {_amount > _wealth}) exitWith
	{
		localize "STR_ZSC_GIVE_FAIL" call dayz_rollingMessages;
	};

	if (!(isPlayer ZSC_GiveMoneyTarget)) exitWith
	{
		format [localize "STR_ZSC_GIVE_PLAYER",CurrencyName] call dayz_rollingMessages;
	};
