sched_playershivewrite =
{
	...
	// КАЖДУЮ 1 МИНУТУ
	// ПРИНУДИТЕЛЬНО БАЗА ДАННЫХ ЗАПИШЕТ ДЛЯ НОВЫХ ИГРОКОВ, КОМУ НАДО БУДЕТ (Человечность или Позиция или ТаймАут изменен)
	...
	if (_n > 0) then
	{
		diag_log format ["[СЕРВЕР] - [sched_playersHivesync.sqf]: СИНХРОНИЗАЦИЯ: %1: Синхронизированно %2 игроков с Базой Данных", __FILE__, _n];
	};
