private ["_object","_worldspace","_location","_dir","_class","_uid","_dam","_hitpoints","_selection","_array","_damage","_fuel","_key","_totaldam","_spawnDMG","_characterID"];
// [_veh,[_dir,_location],"V3S_Civ",true]
#include "\z\addons\dayz_server\compile\server_toggle_debug.hpp"

_object 		=	_this select 0;
_worldspace 	=	_this select 1;
_class 			=	_this select 2;
_spawnDMG 		=	_this select 3;
_characterID 	=	_this select 4;

_fuel 		=	1;
_damage 	=	0;
_array 		=	[];

#ifdef OBJECT_DEBUG
	diag_log ("[СЕРВЕР] - [server_publishVehicle.sqf]: ПУБЛИЧНО: Запрос " + str(_object));
#endif

_dir 		=	_worldspace select 0;
_location 	=	_worldspace select 1;
_uid 		=	_worldspace call dayz_objectUID2;

if (_spawnDMG) then
{ 
	_fuel = 0;
	
	if (getNumber(configFile >> "CfgVehicles" >> _class >> "isBicycle") != 1) then
	{

		// Создаем рандомный урон на детали техники
	
		_totaldam 	= 	0;
		_hitpoints 	= 	_object call vehicle_getHitpoints;
		{
			// Генерируем урон по деталям
			_dam 		= 	call generate_new_damage;
			_selection 	= 	getText(configFile >> "cfgVehicles" >> _class >> "HitPoints" >> _x >> "name");
			
			if (_dam > 0) then
			{
				_array set [count _array,[_selection,_dam]];
				_totaldam = _totaldam + _dam;
			};
		} count _hitpoints;	


		// Если урон будет маленький, то значение может измениться позже
		_damage = 0;

		// Выставляем значение Мин-Макс бензина
		_fuel = (random(DynamicVehicleFuelHigh-DynamicVehicleFuelLow)+DynamicVehicleFuelLow) / 100;

	};
};

// Отправляем запрос
_key = format["CHILD:308:%1:%2:%3:%4:%5:%6:%7:%8:%9:",dayZ_instance, _class, _damage , _characterID, _worldspace, [], _array, _fuel,_uid];
#ifdef OBJECT_DEBUG
	diag_log ("[БАЗА ДАННЫХ] - [server_publishVehicle.sqf]: Запись: "+ str(_key)); 
#endif
_key call server_hiveWrite;

dayz_serverObjectMonitor set [count dayz_serverObjectMonitor,_object];

// Переключаемся на Спавн чтобы подождать ID
[_object,_uid,_fuel,_damage,_array,_characterID,_class] spawn
{
   private["_object","_uid","_fuel","_damage","_array","_characterID","_done","_retry","_key","_result","_outcome","_oid","_selection","_dam","_class"];

   _object 			= 	_this select 0;
   _uid 			= 	_this select 1;
   _fuel 			= 	_this select 2;
   _damage 			= 	_this select 3;
   _array 			= 	_this select 4;
   _characterID 	= 	_this select 5;
   _class 			= 	_this select 6;

   _done 	= 	false;
	_retry 	= 	0;
	
	// ПОТОМ: Требует новой методики! Текущая слишком плоха
	while {_retry < 10} do {
		// Получаем ID из Базы Данных
		_key = format["CHILD:388:%1:",_uid];
		
		#ifdef OBJECT_DEBUG
			diag_log ("[БАЗА ДАННЫХ] - [server_publishVehicle.sqf]: Запись: "+ str(_key));
		#endif
		
		_result 	= 	_key call server_hiveReadWrite;
		_outcome 	= 	_result select 0;
		
		if (_outcome == "PASS") then
		{
			_oid = _result select 1;
			_object setVariable ["ObjectID", _oid, true];
			
			#ifdef OBJECT_DEBUG
				diag_log("[СЕРВЕР] - [server_publishVehicle.sqf]: ВЫБОРКА: Выбрано: " + str(_oid));
			#endif
			
			_done 	= 	true;
			_retry 	= 	100;
		}
		else
		{
			diag_log("[СЕРВЕР] - [server_publishVehicle.sqf]: ВЫБОРКА: Еще раз пробуем получить ID для: " + str(_uid));
			
			_done 	= 	false;
			_retry 	= 	_retry + 1;
			
			uiSleep 1;
		};
	};
	
	if (!_done) exitWith
	{
		deleteVehicle _object;
		
		diag_log("[СЕРВЕР] - [server_publishVehicle.sqf]: ВЫБОРКА: Попытка получить ID для: " + str(_uid));
	};

	_object setVariable ["lastUpdate",time];
	_object setVariable ["CharacterID", _characterID, true];
	_object setDamage _damage;

	// Установим Hits после ObjectID
	{
		_selection 	= 	_x select 0;
		_dam 		= 	_x select 1;
		
		if (_selection in dayZ_explosiveParts && _dam > 0.8) then
		{
			_dam = 0.8
		};
		[_object,_selection,_dam] call fnc_veh_setFixServer;
	} count _array;
	
	_object setFuel _fuel;
	
	_object setvelocity [0,0,1];

	_object call fnc_veh_ResetEH;

	// Тестируется - должен убедиться, что у всех есть обработчик событий для транспортных средств, которые были неиспользованы...
	PVDZE_veh_Init = _object;
	publicVariable "PVDZE_veh_Init";

	diag_log ("[СЕРВЕР] - [server_publishVehicle.sqf]: ПУБЛИЧНО: Создано " + (_class) + " с ID " + str(_uid));
};