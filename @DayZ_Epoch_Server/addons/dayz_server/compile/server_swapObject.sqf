...
{ 
	...
	diag_log ("[СЕРВЕР] - [server_swapObject.sqf]: Неверный объект для Замены (swap) с playerUID:" + (getPlayerUID _activatingplayer));
};

// Публикуем переменную
...
_key = format["CHILD:308:%1:%2:%3:%4:%5:%6:%7:%8:%9:",dayZ_instance, _class, 0 , _charID, _worldspace call AN_fnc_formatWorldspace, _inv, [], 0,_uid]; 		// KK_Functions.sqf

//diag_log ("HIVE: WRITE: "+ str(_key));		// Позже
...
diag_log ("[СЕРВЕР] - [server_swapObject.sqf]: ПУБЛИЧНО: " + str(_activatingPlayer) + " Улучшен " + (_class) + " с ID " + str(_uid));
